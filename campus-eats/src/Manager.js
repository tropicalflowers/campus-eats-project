...
const RestaurantManagementDrawer = ({ restaurants, addRestaurant, toggleRestaurant, addMenuItem }) => {
  ...
  const handleAddRestaurant = () => {
    if (!newRestaurant.name || !newRestaurant.type) {
      alert('Please fill in restaurant name and type!');
      return;
    }
    addRestaurant(newRestaurant);
    setNewRestaurant({ name: '', type: '', description: '' });
  };

  const handleAddMenuItem = () => {
    if (!selectedRestaurant || !newMenuItem.name || !newMenuItem.price) {
      alert('Please select restaurant and fill in item details!');
      return;
    }
    addMenuItem(selectedRestaurant, newMenuItem);
    setNewMenuItem({ name: '', description: '', price: '', section: 'main' });
  };
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-500/10 to-cyan-500/10 p-6 rounded-xl border border-blue-500/30">
        <h3 className="text-2xl font-bold mb-4 bg-gradient-to-r from-blue-400 to-cyan-400 bg-clip-text text-transparent">➕ Add New Restaurant</h3>
        <div className="space-y-3">
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-emerald-500/80 to-teal-500/80 text-white py-3 rounded-xl font-bold hover:from-emerald-600/80 hover:to-teal-600/80 transition shadow-lg"
          >
            Add Restaurant
          </button>
        </div>
      ...
      <div className="bg-gradient-to-r from-amber-500/10 to-orange-500/10 p-6 rounded-xl border border-amber-500/30">
        <h3 className="text-2xl font-bold mb-4 bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent">🏪 Manage Restaurants</h3>
        <div className="space-y-3">
          {restaurants.map((restaurant) => (
            <div key={restaurant.id} className="flex items-center justify-between p-4 bg-slate-800/60 rounded-xl border border-slate-600/40">
              <div>
                <p className="font-bold text-white text-lg">{restaurant.name}</p>
                <p className="text-sm text-slate-400">{restaurant.type}</p>
              </div>
              <button
                onClick={() => toggleRestaurant(restaurant.id, restaurant.isOpen)}
      ...
      <div className="bg-gradient-to-r from-purple-500/10 to-pink-500/10 p-6 rounded-xl border border-purple-500/30">
        <h3 className="text-2xl font-bold mb-4 bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">🍔 Add Menu Item</h3>
        <div className="space-y-3">
          <select
            value={selectedRestaurant}
            onChange={(e) => setSelectedRestaurant(e.target.value)}
            className="w-full px-4 py-3 rounded-xl bg-slate-800/60 border border-slate-600/40 text-white focus:border-purple-500/60 focus:outline-none"
          >
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-purple-500/80 to-pink-500/80 text-white py-3 rounded-xl font-bold hover:from-purple-600/80 hover:to-pink-600/80 transition shadow-lg"
          >
            Add Menu Item
          </button>
        </div>
...
function Manager() {
  const { state, showMessage, openDrawer, closeDrawer, updateCart, dispatch, dbFunctions } = useAppContext();
  const { cart, isLightMode, user, restaurants: stateRestaurants } = state;
  ...
  const restaurants = stateRestaurants?.length > 0 ? stateRestaurants : localRestaurants;
  ...
  const addRestaurant = async (restaurantData) => {
    ...
    if (dbFunctions?.addRestaurant) {
      await dbFunctions.addRestaurant(newRestaurant);
      showMessage(`${restaurantData.name} added successfully!`, 'success');
    }
  };

  const toggleRestaurant = async (id, currentStatus) => {
    if (dbFunctions?.toggleRestaurantStatus) {
      await dbFunctions.toggleRestaurantStatus(id, currentStatus);
  ...
  const addMenuItem = async (restaurantId, item) => {
    ...
    if (dbFunctions?.addDishToMenu) {
      await dbFunctions.addDishToMenu(restaurantId, menuItem, item.section);
  ...
  const bgClass = isLightMode 
    ? 'bg-gradient-to-br from-violet-50/60 via-purple-50/60 to-fuchsia-50/60' 
    : 'bg-gradient-to-br from-slate-950 via-purple-950/60 to-indigo-950/60';
  const textClass = isLightMode ? 'text-gray-800' : 'text-slate-100';
  const cardClass = isLightMode 
    ? 'bg-white/70 backdrop-blur-sm border-purple-100/40' 
    : 'bg-slate-900/60 backdrop-blur-sm border-purple-900/30';
  ...
  return (
    <div className={`min-h-screen ${bgClass} ${textClass} transition-all duration-500`}>
      ...
      <div className="sticky top-0 z-40 backdrop-blur-xl border-b shadow-xl transition-all duration-300" 
        style={{ 
          backgroundColor: isLightMode ? 'rgba(250,245,255,0.85)' : 'rgba(15,23,42,0.85)',
          borderColor: isLightMode ? 'rgba(233,213,255,0.2)' : 'rgba(147,51,234,0.2)'
        }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              <Link to="/" className={`text-2xl font-extrabold ${isLightMode ? 'bg-gradient-to-r from-violet-500/80 via-purple-500/80 to-fuchsia-500/80' : 'bg-gradient-to-r from-violet-400/80 via-purple-400/80 to-fuchsia-400/80'} bg-clip-text text-transparent hover:scale-105 transition-transform`}>
                Campus Eats
              </Link>
              <div className="hidden sm:flex items-center gap-2">
                <span className={`text-sm ${isLightMode ? 'text-purple-500/70' : 'text-purple-400/70'}`}>Manager</span>
                {user?.email && (
                  <>
                    <span className="text-slate-400/60">•</span>
                    <span className={`text-sm font-semibold ${isLightMode ? 'bg-gradient-to-r from-violet-500/70 to-purple-500/70' : 'bg-gradient-to-r from-violet-400/70 to-purple-400/70'} bg-clip-text text-transparent notranslate`}>{user.email}</span>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`p-2 px-4 rounded-xl transition-all flex items-center gap-2 ${
                  isLightMode 
                    ? 'bg-gradient-to-r from-violet-100/60 to-purple-100/60 hover:from-violet-200/60 hover:to-purple-200/60 shadow-md' 
                    : 'bg-gradient-to-r from-purple-900/60 to-indigo-900/60 hover:from-purple-800/60 hover:to-indigo-800/60 shadow-lg shadow-purple-900/30'
                }`}
                title="Go to Home"
              >
              ...
              <button onClick={openCart} className={`${isLightMode ? 'bg-gradient-to-r from-violet-400/70 via-purple-400/70 to-fuchsia-400/70 hover:from-violet-500/70 hover:via-purple-500/70 hover:to-fuchsia-500/70' : 'bg-gradient-to-r from-violet-500/70 via-purple-500/70 to-fuchsia-500/70 hover:from-violet-600/70 hover:via-purple-600/70 hover:to-fuchsia-600/70'} text-white px-6 py-2 rounded-xl font-bold transition-all flex items-center gap-2 shadow-lg hover:shadow-xl`}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className={`lg:col-span-2 ${cardClass} border rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-300`}>
            <p className={`text-sm font-bold mb-2 ${isLightMode ? 'bg-gradient-to-r from-violet-500/70 to-purple-500/70' : 'bg-gradient-to-r from-violet-400/70 to-purple-400/70'} bg-clip-text text-transparent`}>👋 Welcome Manager, <span className="notranslate">{user?.email?.split('@')[0] || 'Guest'}</span>!</p>
            <h1 className={`text-5xl font-extrabold mb-4 ${isLightMode ? 'bg-gradient-to-r from-violet-500/80 via-purple-500/80 to-fuchsia-500/80' : 'bg-gradient-to-r from-violet-500/80 via-purple-500/80 to-fuchsia-500/80'} bg-clip-text text-transparent`}>Full Admin Control</h1>
            <p className={`text-lg mb-6 ${isLightMode ? 'text-gray-600' : 'text-slate-300'}`}>
              Manage restaurants, book mess meals, and place orders - all with full privileges!
            </p>
            <div className="grid grid-cols-2 gap-4">
              <button onClick={openRestaurantManagement} className={`${isLightMode ? 'bg-gradient-to-r from-violet-400/80 to-purple-400/80 hover:from-violet-500/80 hover:to-purple-500/80' : 'bg-gradient-to-r from-violet-500/80 to-purple-500/80 hover:from-violet-600/80 hover:to-purple-600/80'} text-white px-6 py-4 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl`}>
                ⚙️ Manage Restaurants
              </button>
              <button onClick={openMessBooking} className={`${isLightMode ? 'bg-gradient-to-r from-amber-400/80 to-orange-400/80 hover:from-amber-500/80 hover:to-orange-500/80' : 'bg-gradient-to-r from-amber-500/80 to-orange-500/80 hover:from-amber-600/80 hover:to-orange-600/80'} text-white px-6 py-4 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl`}>
                🍽️ Book Mess
              </button>
            </div>
          </div>
          ...
          {/* Wallet Card */}
          <div className={`${cardClass} border rounded-3xl p-8 shadow-xl ${isLightMode ? 'bg-gradient-to-br from-violet-300/60 via-purple-300/60 to-fuchsia-300/60' : 'bg-gradient-to-br from-violet-500/60 via-purple-500/60 to-fuchsia-500/60'} hover:shadow-2xl transition-all duration-300`}>
          ...
          <div className={`${cardClass} border rounded-3xl p-6 mb-8 shadow-xl`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {messBookings.slice(-6).reverse().map((booking) => (
                <div key={booking.id} className={`p-4 rounded-xl ${isLightMode ? 'bg-violet-50/60' : 'bg-purple-900/20'} border ${isLightMode ? 'border-violet-200/40' : 'border-purple-800/30'}`}>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {booking.meals.map((meal) => (
                      <span key={meal} className={`text-xs px-2 py-1 rounded-full ${isLightMode ? 'bg-violet-200/60 text-violet-800' : 'bg-purple-800/60 text-purple-200'}`}>
        ...
        <h2 className={`text-4xl font-extrabold mb-8 ${textClass} flex items-center gap-3`}>
          <span className={`${isLightMode ? 'bg-gradient-to-r from-violet-500/80 via-purple-500/80 to-fuchsia-500/80' : 'bg-gradient-to-r from-violet-500/80 via-purple-500/80 to-fuchsia-500/80'} bg-clip-text text-transparent`}>🍔 Today's Spots</span> 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 mb-16">
            {openRestaurants.map((restaurant) => (
              <div key={restaurant.id} className={`${cardClass} border rounded-3xl p-6 hover:shadow-xl transition-all duration-300 hover:scale-105 cursor-pointer group`} onClick={() => openMenu(restaurant)}>
                <div className={`w-full h-36 ${isLightMode ? 'bg-gradient-to-br from-violet-300/70 via-purple-300/70 to-fuchsia-300/70' : 'bg-gradient-to-br from-violet-400/70 via-purple-400/70 to-fuchsia-400/70'} rounded-2xl mb-5 flex items-center justify-center text-5xl group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                  🍽️
                </div>
                <h2 className={`text-2xl font-bold mb-2 ${isLightMode ? 'bg-gradient-to-r from-violet-500/80 to-purple-500/80' : 'bg-gradient-to-r from-violet-500/80 to-purple-500/80'} bg-clip-text text-transparent notranslate`}>{restaurant.name}</h2>
                ...
                <button className={`w-full ${isLightMode ? 'bg-gradient-to-r from-violet-400/80 to-purple-400/80 hover:from-violet-500/80 hover:to-purple-500/80' : 'bg-gradient-to-r from-violet-500/80 to-purple-500/80 hover:from-violet-600/80 hover:to-purple-600/80'} text-white py-3 rounded-xl font-bold transition-all shadow-md group-hover:shadow-lg`}>
                  View Menu
                </button>
              </div>
            ))}
            ...
            <div className={`${cardClass} border rounded-3xl overflow-hidden shadow-xl`}>
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  <thead className={isLightMode ? 'bg-gradient-to-r from-violet-50/60 to-purple-50/60' : 'bg-gradient-to-r from-purple-900/40 to-indigo-900/40'}>
                    <tr>
                  ...
                  <tbody>
                    {userOrders.map((order, idx) => (
                      <tr key={order.id} className={`border-t ${isLightMode ? 'border-violet-100/30 hover:bg-violet-50/40' : 'border-purple-900/20 hover:bg-purple-900/20'} transition-all`}>
                        <td className="p-5 font-semibold">{idx + 1}</td>
                        <td className="p-5 text-sm opacity-90">{new Date(order.timestamp).toLocaleString()}</td>
                        <td className={`p-5 font-bold ${isLightMode ? 'bg-gradient-to-r from-violet-500/70 to-purple-500/70' : 'bg-gradient-to-r from-violet-400/70 to-purple-400/70'} bg-clip-text text-transparent`}>{order.mode}</td>
